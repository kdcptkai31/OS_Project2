...
int main() {
  ...
  while (true) {
    ...
    cout << "***error*** invalid input! Please enter again\n";
  ...
  while (1) {
    ...
    if (waitList.size == 0 && frame_list_is_empty(frameList)) break;
...
void add_new_proc_to_waitList(int current_time) {
  ...
  for (int i = 0; i < number_of_processes; i++) {
    ...
    if (proc.arriving_time == current_time) {
      ...
      cout << print_time << "Process " << proc.pid << " arrives\n";
...
void remove_complete_process(int current_time) {
  ...
  for (i = 0; i < number_of_processes; i++) {
	...
	time_spent_in_memory >= processList[i].burst_time) {

      cout << correct_format_for_time(current_time)  << "Process "
	   << processList[i].pid << " completes\n";
      processList[i].is_active = 0;
